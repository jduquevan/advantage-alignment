...
def run_job(command, fake_submit: bool = True):
    ...
    if fake_submit:
        ...
        command = f'sbatch --time=24:0:0  --gres=gpu:a100:1 --account=rrg-bengioy-ad --cpus-per-task=6 --mem=40G --gres=gpu:1 -c 4 sweep_temp_job.sh  '
        os.system(command)
...
def run_random_jobs(fake_submit: bool = True):
    command = ("python train.py seed=42 "
               ...
               "wandb_tags=[commons_harvest,v105] "
    ...
    cfg['optimizer_ss__lr'] = 0.0 # no self-supervised loss
