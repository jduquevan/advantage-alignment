...
def compute_discounted_returns(gamma, rewards, agent):
    ...
    returns = torch.empty(
        (batch_size, trajectory_len), device=agent.device, dtype=torch.float32
    )
    returns[:, -1] = rewards[:, -1]
...
def instantiate_agent(cfg: DictConfig):
    ...
    mlp_model = hydra.utils.instantiate(
        cfg.mlp_model, gru=gru_model, transformer=transformer
    )

    actor = hydra.utils.instantiate(cfg.policy, model=mlp_model)
    critic = hydra.utils.instantiate(
        cfg.linear_model, gru=gru_model, transformer=transformer
    )

    target = hydra.utils.instantiate(
        cfg.linear_model, gru=gru_target, transformer=transformer_t
    )

    optimizer_actor = hydra.utils.instantiate(
        cfg.optimizer_actor, params=actor.parameters()
    )
    optimizer_critic = hydra.utils.instantiate(
        cfg.optimizer_critic, params=critic.parameters()
    )
...
def wandb_stats(trajectory):
    ...
    stats["Average sum rewards"] = (
        (trajectory.data["rewards_0"] + trajectory.data["rewards_1"]).mean().detach()
    )
    stats["Average traj len"] = (
        (dones.size(0) * dones.size(1)) / torch.sum(dones)
    ).detach()
